...
class TodoManager:
    """Manage a list of todo items with status tracking.

    Each item has an id, text, and one of three statuses:
    pending, in_progress, or completed.
    """

    def __init__(self) -> None:
        """Initialize with an empty todo list."""
        self.items = []
    ...
    def update(self, items: list) -> str:
        ...
        - Each item has valid text and status
        - Only one item is in_progress at a time

        Args:
            items: A list of todo item dicts.

        Returns:
            A rendered string of the updated todo list.
        """
        if len(items) > 20:
    ...
    def render(self) -> str:
        ...
        Returns:
            A human-readable representation with status markers.
        """
        if not self.items:
            return "No todos!"
        lines = []
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        if rounds_since_todo >= 3:
            results.append({"type": "text", "text": "<reminder>Update your todo list to track progress and stay on course!</reminder>"})
