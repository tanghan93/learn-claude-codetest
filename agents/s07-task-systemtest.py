...
任务以 JSON 文件存在 .tasks/ 中，即使上下文压缩也不会消失。
每个任务可以有依赖（blockedBy）。

例：
task_1.json → 完成
task_2.json → 依赖 task_1
task_3.json → 依赖 task_2

任务1完成 → 任务2自动解除依赖

关键思想：状态存在对话之外，所以压缩上下文也不会丢失！
"""
...
class TaskManager:
    """Manage tasks with dependencies, persisted as JSON files in .tasks/."""
   ...
   # 获取当前最大任务 ID
    ...
    # 创建新任务，自动分配 ID，初始状态为 pending，保存到文件
    ...
    # 更新任务状态、依赖等信息，完成后自动解除依赖
    def update(self, task_id: int, status=None, add_blocked_by=None, remove_blocked_by=None):
        task = self._load(task_id)
        if status:
            task["status"] = status
            if status == "completed":
                self._clear_dependency(task_id)  # 完成后自动解除别人的依赖
        if add_blocked_by:
            task["blockedBy"] += add_blocked_by
        if remove_blocked_by:
            task["blockedBy"] = [x for x in task["blockedBy"] if x not in remove_blocked_by]
        self._save(task)
        return json.dumps(task, indent=2)
    
    # 清楚依赖：当一个任务完成时，遍历所有任务，把它从 blockedBy 中删掉
    def _clear_dependency(self, completed_id: int):
        # 遍历所有任务，把 completed_id 从 blockedBy 中删掉
...
# 实例化 TaskManager，创建全局任务管理器，供工具函数调用    
